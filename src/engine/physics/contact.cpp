...
void Contact::calculateInternals(float deltaTime)
{
    ...
    m_relativeContactPosition[0] =
        m_contactPoint - m_body[0]->m_entity->GetComponent<Transform>()->getPosition();

    if (m_body[1])
        m_relativeContactPosition[1] =
            m_contactPoint - m_body[1]->m_entity->GetComponent<Transform>()->getPosition();

    m_contactVelocity = calculateLocalVelocity(0, deltaTime);
    if (m_body[1]) m_contactVelocity -= calculateLocalVelocity(1, deltaTime);
...
void Contact::calculateDesiredDeltaVelocity(float deltaTime)
{
    ...
    if (fabsf(m_contactVelocity.x) < velocityLimit) thisRestitution = 0.0f;

    m_desiredDeltaVelocity =
        -m_contactVelocity.x - thisRestitution * (m_contactVelocity.x - velocityFromAcc);
}
...
void Contact::applyPositionChange(glm::vec3 linearChange[2], glm::vec3 angularChange[2],
                                  float penetration)
{
    ...
    for (size_t i = 0; i < 2; i++)
    {
        ...
        else
        {
            glm::vec3 targetAngularDirection =
                glm::cross(m_relativeContactPosition[i], m_contactNormal);
            glm::mat3 inverseInertiaTensor = m_body[i]->m_invInertiaTensorWorld;
...
void Contact::applyVelocityChange(glm::vec3 velocityChange[2], glm::vec3 rotationChange[2])
{
    ...
    if (m_body[1]) inverseInertiaTensor[1] = m_body[1]->m_invInertiaTensorWorld;
...
glm::vec3 Contact::calculateFrictionlessImpulse()
{
    ...
    glm::vec3 impulseContact = glm::vec3(m_desiredDeltaVelocity / deltaVelocity, 0.0f, 0.0f);
...
glm::vec3 Contact::calculateFrictionImpulse()
{
    ...
    glm::mat3 impulseToTorque(0.0f, m_relativeContactPosition[0].z, -m_relativeContactPosition[0].y,
                              -m_relativeContactPosition[0].z, 0.0f, m_relativeContactPosition[0].x,
                              m_relativeContactPosition[0].y, -m_relativeContactPosition[0].x,
                              0.0f);
